...
describe('Html', () => {
  describe('<require>', () => {
    ...
    it('works on "write" ops', () => {
      ...
      const modPlugin = ModifyPlugin.searchReplace(/\.html$/, modStr);
      const cfg = choseLocal({ plugins: [modPlugin, Html()] });
      const exp = '<script src="/out-hello.js"></script>'
    ...
    it('follows renames', () => {
      const modPlugin = ModifyPlugin.rename(/\.js$/, f => "renamed-" + f);
      const cfg = choseLocal({ plugins: [modPlugin, Html()] });
    ...
    it('error when file type unrecognised', () => {
      const modPlugin = ModifyPlugin.rename(/\.js$/, f => f + ".what");
      const cfg = choseLocal({ plugins: [modPlugin, Html()] });
